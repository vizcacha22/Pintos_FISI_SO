...
bool compare_priority_thread(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED);
...
void thread_yield(void)
{
  ...
  if (cur != idle_thread)
    list_insert_ordered(&ready_list, &cur->elem, compare_priority_thread, NULL);
...
void thread_sleep(int64_t ticks)
{
  ...
  list_insert_ordered(&sleep_list, &cur->elem, compare_wakeup_tick, 0);
...
void thread_set_priority(int new_priority)
{
  ...
  t->init_priority = new_priority; // Set new priority.
...
void mlfqs_calculate_priotity(struct thread *t)
{
  if (t == idle_thread)
    return;

  int priority = fixed_point_to_int_round(add_fixed_point_int(div_fixed_point_int(t->recent_cpu, -4), PRI_MAX - t->nice * 2));
...
{
  t->recent_cpu = add_fixed_point_int(mult_fixed_point(div_fixed_point(mult_fixed_point_int(load_avg, 2), add_fixed_point_int(mult_fixed_point_int(load_avg, 2), 1)), t->recent_cpu), t->nice);
}
...
void mlfqs_calculate_load_avg(void)
{
  ...
  load_avg = add_fixed_point(mult_fixed_point(div_fixed_point_int(int_to_fixed_point(59), 60), load_avg),
                             mult_fixed_point_int(div_fixed_point_int(int_to_fixed_point(1), 60), ready_threads));
}
...
void mlfqs_increment_recent_cpu(void)
{
  struct thread *t = thread_current();
  if (t != idle_thread)
    t->recent_cpu = add_fixed_point_int(thread_current()->recent_cpu, 1);
}

/* Recalculate and update all threads' MLFQS priority and recent_cpu. */
void mlfqs_calculate_update_recent_cpu(void)
{
  struct list_elem *e;
...
/* Recalculate and update all threads' MLFQS priority and recent_cpu. */
void mlfqs_calculate_update_priority(void)
{
  struct list_elem *e;
  ...
  {
    struct thread *t = list_entry(e, struct thread, allelem);
    mlfqs_calculate_priotity(t);
  }

  list_sort(&ready_list, compare_priority_thread, NULL);
}
...
void thread_donate_priority(void)
{
  ...
  while (cur->locks_waiting != NULL && depth < 8)
  {
    struct lock *waiting_lock = cur->locks_waiting;
...
void thread_update_priority(void)
{
  ...
  if (!list_empty(&t->prio_donation))
  {
    struct thread *top_donor = list_entry(list_front(&t->prio_donation), struct thread, prio_donation_elem);
...
int thread_get_load_avg(void)
{
  ...
  int new_load_avg = fixed_point_to_int_round(mult_fixed_point_int(load_avg, 100));
...
int thread_get_recent_cpu(void)
{
  ...
  int recent_cpu = fixed_point_to_int_round(mult_fixed_point_int(thread_current()->recent_cpu, 100));
